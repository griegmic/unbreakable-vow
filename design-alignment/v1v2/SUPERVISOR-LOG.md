...
- PR #3Z — Witness landing polish (/w/[token]) — Self-eval 8/8 YES — Brand treatment: EyebrowTag replaced with diamond seal + serif wordmark per mock. H1 copy: "needs you to hold them to this." Role line: serif italic per mock. Decline button: swapped to MutedSecondary primitive. Added "First time? How this works" footer. Doc card: rebuilt with mock-exact CSS (6px radius, gold gradient line, 21px quote, grid meta with tnum). Padding: 30px 24px 26px per mock. Cleaned unused imports. Files: `web/src/app/w/[token]/client.tsx`.
