...
export default function VerdictNotFound({ token }: { token: string }) {
  const [vow, setVow] = useState<any>(null);
  const [alreadyJudged, setAlreadyJudged] = useState<string | null>(null);
      ...
      <div className="min-h-dvh flex items-center justify-center" style={{ background: 'var(--bg)' }}>
        <div className="text-center px-6">
          ...
          <p className="text-[15px]" style={{ color: 'var(--text-secondary)' }}>
            This vow has already been judged: <span className="font-semibold" style={{ color: alreadyJudged === 'kept' ? 'var(--success)' : 'var(--danger)' }}>{alreadyJudged}</span>.
          </p>
        </div>
      </div>
    );
