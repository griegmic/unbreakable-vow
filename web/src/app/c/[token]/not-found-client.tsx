...
export default function ChallengeNotFound({ token }: { token: string }) {
  ...
  if (vow) {
    return <ChallengeInviteClient vow={vow} token={token} makerName="Your friend" makerPhone={null} />;
  }
    ...
    return (
      <div className="min-h-dvh flex items-center justify-center" style={{ background: 'var(--bg)' }}>
        <div className="text-center px-6 flex flex-col items-center gap-4">
          <h1 className="text-2xl font-serif font-bold mb-2" style={{ color: 'var(--text)' }}>Challenge not found</h1>
          <p className="text-[15px]" style={{ color: 'var(--text-secondary)' }}>
            This link may have expired or is invalid. Ask the challenger to send it again.
          </p>
          <button
