...
const DIR = path.join(__dirname, '../renders/pr3k');
const mock = PNG.sync.read(fs.readFileSync(path.join(DIR, 'mock-refine.png')));
const impl = PNG.sync.read(fs.readFileSync(path.join(DIR, 'impl-refine.png')));
...
// Mask bottom 180px (dev badge at 3x)
...
fs.writeFileSync(path.join(DIR, 'diff-refine.png'), PNG.sync.write(diff));
